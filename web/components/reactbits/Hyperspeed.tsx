...
const Hyperspeed = ({ effectOptions = {}, className = '' }: HyperspeedProps) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const appRef = useRef<any>(null);
  const uid = useId();

  const defaults: HyperspeedEffectOptions = {
    onSpeedUp: () => {},
    onSlowDown: () => {},
  ...
  useEffect(() => {
    ...
    function resizeRendererToDisplaySize(renderer: THREE.WebGLRenderer, container: HTMLDivElement) {
    ...
    class App {
      ...
      constructor(ctr: HTMLDivElement, options: any) {
        ...
        this.camera = new THREE.PerspectiveCamera(options.fov, ctr.offsetWidth / ctr.offsetHeight, 0.1, 10000);
