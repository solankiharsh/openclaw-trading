  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        float upperFade = clamp(mapRange(0.0, 1.0, 0.0, 1.0, transformedUV.y + 0.10), 0.0, 1.0);

        vec2 vignetteCoords = (screenUV - 0.5) * 2.0;
        float vignetteRadius = length(vignetteCoords);
        float vignetteMask = smoothstep(-1.4 + uCircleScale, 0.2 + uCircleScale, 1.0 - vignetteRadius);
